...
const Navbar = () => {
  ...
  const totalCars = useCartStore((state) =>
  ...
  return (
    <div
      className={` flex justify-between items-center p-4 md:px-10  w-full top-0 z-50 transition-all duration-300 ${
        scrolled
          ? "fixed bg-white text-gray-800 shadow-md" // After scrolling
          : "md:absolute fixed md:bg-transparent bg-white md:text-white text-gray-800" // Initial style
      }`}
    >
      <h1 className="font-bold text-5xl ">
        Your<span className="font-thin">Car</span>
      </h1>

      <div className=" flex font-bold text-2xl">
        <div className="md:block hidden ">
          {["home", "about", "services", "cars", "contact"].map((title) => (
            <a
              key={title}
              href={`#${title}`}
            ...
            ${
              active === title
                ? "bg-[#741906] text-white scale-105" // Active styles
                : "hover:text-[#741906]  hover:scale-105" // Hover styles
            }`}
            >
        ...
        <div className="flex">
          <button className="px-6 relative" onClick={toggleCart}>
            <svg
              width="40"
              height="35"
              viewBox="0 0 35 30"
            ...
            <h1 className="absolute bg-[#F8D7A4] w-6 h-6 font-semibold text-base text-[#223F5C] rounded top-0 right-5 ">
              {totalCars}
            </h1>
          </button>
